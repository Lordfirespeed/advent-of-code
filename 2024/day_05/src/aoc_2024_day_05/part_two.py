...
class PartTwoSolver:
    ...
    def reordered_incorrectly_ordered_updates(self) -> Iterable[Sequence[int]]:
        for update in self.instance.updates:
            order = self.page_order(self.instance.page_ordering_rules, update)
            if self.is_correctly_ordered(update, order):
                continue
            reordered_update = sorted(update, key=lambda x: order.index(x))
            yield reordered_update
    ...
    def solve(self) -> int:
        return sum(self.middle_page_number(update) for update in self.reordered_incorrectly_ordered_updates())
